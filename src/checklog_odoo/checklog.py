...
def _render_skipped_tests(skipped_test_messages):
    msg = [
        click.style(
    ...
    msg.extend(f"{message}\n" for message in skipped_test_messages)
    return "".join(msg)
...
def do_checklog(filename, ignore, echo, *, err_if_empty=True, err_on_skipped=True):
    ...
    with click.open_file(filename) as logfile:
        ...
        if not reccount and not skipped_test_messages and err_if_empty:
        ...
        if skipped_test_messages:
            msg = _render_skipped_tests(skipped_test_messages)
            click.echo(msg)
        if error_records:
...
def checklog_odoo(
    ...
    do_checklog(filename, ignore, echo, err_if_empty=err_if_empty, err_on_skipped=err_on_skipped)
