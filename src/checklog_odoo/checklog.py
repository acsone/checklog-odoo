...
PYTEST_SKIPPED_RE = re.compile(r"(?:^|\s)SKIPPED\s+(?P<nodeid>\S+\.py::\S+)")
...
def _get_skipped_test_message(line, mo):
    ...
    if skipped_match:
        return f"SKIPPED {skipped_match.group('nodeid')}"
    return None
...
def do_checklog(filename, ignore, echo, *, err_if_empty=True, err_on_skipped=True, allowed_skipped=()):
    ...
    allowed_skipped = [i for i in allowed_skipped if not i.startswith("#")]
    allowed_skipped_regexes = [re.compile(i, re.MULTILINE) for i in allowed_skipped]
    ...
    with click.open_file(filename) as logfile:
        ...
        for line in logfile:
            ...
            if skipped_test_message and skipped_test_message not in skipped_test_message_set:
                ...
                if not any(regex.search(skipped_test_message) for regex in allowed_skipped_regexes):
...
def checklog_odoo(ctx, filename, config, ignore, verbose, echo, err_if_empty, err_on_skipped):
    ...
    allowed_skipped = checklog_config.get("allowed_skipped")
...
def _read_defaults(config):
    ...
    defaults = {
        ...
        "allowed_skipped": config.getlist(section, "allowed_skipped", []),
    }
